...
fail() { printf '[!] %s\n' "$*" >&2; exit 1; }
required=(INTERFACE PI_IP_CIDR PI_IP ROUTER_IP DHCP_START DHCP_END DHCP_NETMASK DHCP_LEASE_TIME UPSTREAM_DNS_1 UPSTREAM_DNS_2 TIMEZONE PIHOLE_PASSWORD GRAFANA_ADMIN_USER GRAFANA_ADMIN_PASSWORD TAILSCALE_HOSTNAME DHCP_HOST_1 DHCP_HOST_2 DHCP_HOST_3 MEDIA_MOUNT_DIR MEDIA_DISK_UUID MEDIA_DISK_TYPE JELLYFIN_HTTP_PORT JELLYFIN_HTTPS_PORT GRAFANA_PORT PIHOLE_IMAGE TAILSCALE_IMAGE JELLYFIN_IMAGE GRAFANA_IMAGE PROMETHEUS_IMAGE NODE_EXPORTER_IMAGE CADVISOR_IMAGE BLACKBOX_IMAGE PIHOLE_EXPORTER_IMAGE)
for name in "${required[@]}"; do
    [[ -n "${!name:-}" ]] || fail "Brak wymaganej wartosci: ${name}"
done

[[ "$INTERFACE" =~ ^[[:alnum:]_.:-]+$ ]] || fail "INTERFACE zawiera niedozwolone znaki."
